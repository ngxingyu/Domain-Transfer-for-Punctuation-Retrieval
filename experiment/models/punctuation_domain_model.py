...
class PunctuationDomainModel(pl.LightningModule, Serialization, FileIO):
    '''Pytorch Lightning Module for Punctuation Domain Transfer.'''
    def __init__(self,
                 cfg: DictConfig,
        ...
        assert(self._cfg.model.dataset.max_seq_length%self._cfg.model.dataset.stride==0,'ensure stride divides max_seq_length')
        if self._cfg.model.mask.type=='uniform':
            self._cfg.model.mask.sigma=self._cfg.model.dataset.max_seq_length/self._cfg.model.dataset.stride

        self.mask=test_mask(self._cfg.model.mask.type,
                self._cfg.model.mask.sigma,
                self.tokenizer,
                self.labels_to_ids,self.label_map,
                self._cfg.model.dataset.max_seq_length,self._cfg.model.dataset.stride,
                self._cfg.model.dataset.attach_label_to_end,
                self._cfg.model.no_space_label)                
        ...
        self.domain_classifier = TokenClassifier(
        ...
        self.domain_class_report = ClassificationReport(
            num_classes=2 if self.hparams.model.domain_head.predict_labelled else\
                self.hparams.model.dataset.num_domains,
            label_ids={v:v for v in list(range(2 if \
                self.hparams.model.domain_head.predict_labelled else\
                self.hparams.model.dataset.num_domains))},
            mode='macro',
            dist_sync_on_step=True,
        )
    ...
    def _make_step(self, batch):
        ...
        punct_logits_pred=punct_logits.argmax(axis=-1)
        token_weight=F.one_hot(punct_logits_pred,num_classes=punct_logits.shape[-1])#*punct_logits
        if self._cfg.model.punct_class_weights is not None:
            token_weight=token_weight*torch.tensor(self._cfg.model.punct_class_weights).type_as(punct_logits)
        idx = punct_logits_pred.unsqueeze(2)
        token_weight = token_weight.gather(2, idx)
        token_weight = token_weight.squeeze(2)
            
        if (self.hparams.model.domain_head.loss in ['focal','cel']) and (self.hparams.model.domain_head.weight_tokens==True)\
            and (self.hparams.model.domain_head.pooling is None):
            domain_loss = self.domain_loss(
            logits=domain_logits, labels=domain_labels.repeat(1,punct_labels.shape[-1]), token_weight=token_weight)
        else:
            domain_loss = self.domain_loss(
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (batch_idx%1000==0):
            print('gamma:',self.grad_reverse.scale)
    ...
    def multi_test_epoch_end(self, outputs):
        ...
        path=f"{self.hparams.log_dir}/test-{''.join([_.split('/')[-1][:3] for _ in self.dm.test_dataset.labelled])}-{self.frozen}.txt" if self.hparams.log_dir!='' else f'{self.hparams.exp_manager.exp_dir}{self.hparams.exp_manager.name}'
    ...
    def add_punctuation(self, queries):
        ...
        subtoken_mask = batch['subtoken_mask'].to(device)
        # punct_labels = batch['labels']
