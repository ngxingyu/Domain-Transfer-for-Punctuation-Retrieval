...
def combinepreds(preds,input_ids,subtoken_mask,mask,stride,labels=None,num_labels=8):
    ...
    for i in range(len(preds)):
        print(preds[i][1:-1].shape,combined_result[offset:offset+len(mask)].shape)
...
result,label,combinedmask=combinepreds(F.one_hot(batched['labels'],num_classes),batched['input_ids'],batched['subtoken_mask'],mask.unsqueeze(-1),stride,batched['labels'],num_classes)
